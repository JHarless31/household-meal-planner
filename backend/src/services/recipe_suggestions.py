...
class RecipeSuggestionService:
    ...
    def suggest_by_rotation(
        ...
        for recipe in recipes:
            ...
            suggestions.append({
                ...
                "last_cooked_date": recipe.last_cooked_date.isoformat() if recipe.last_cooked_date else None,
    ...
    def suggest_never_tried(
        ...
        recipes = db.query(Recipe).filter(
            ...
            Recipe.created_at.desc()  # Newest recipes first
        ).limit(limit).all()
    ...
    def suggest_by_available_inventory(
        ...
        available_ingredients = {item.item_name.lower().strip() for item in inventory_items}
        ...
        for recipe in recipes:
            ...
            match_percent = matched_ingredients / total_ingredients if total_ingredients > 0 else 0
            ...
            if match_percent >= min_ingredient_match_percent:
                suggestions.append({
                    ...
                    "missing_ingredients": missing_ingredients[:3],  # First 3 missing
    ...
    def suggest_seasonal(
        ...
        recipes = db.query(Recipe).join(
            RecipeTag, Recipe.id == RecipeTag.recipe_id
        ).filter(
            Recipe.is_deleted == False,
            RecipeTag.tag.ilike(f"%{season}%")
        ).limit(limit).all()
